...
- [ x] Explain the differences between `client-side routing` and `server-side routing`.
  With client-side routing, you download less data to display new content. Instead of downloading the entire page every time new data arises, you download specific data that you requested. It can be a faster, more enjoyable user exeperience. However, client side routing does not have SEO quite the same as server-side routing does.

Server side routing is when the brower makes an HTTP GET request to a certain URL, the server responds with an entire webpage that discards the previous webpage. It takes a little longer, but has better SEO.
